...
const OTPModal = () => {
  ...
  return (
    <Dialog open={otpRequired} onOpenChange={(open) => !loading && setOtpRequired(open)}>
      <DialogContent className="sm:max-w-md bg-background text-foreground border-border shadow-2xl p-0 overflow-y-auto max-h-[95vh] sm:max-h-none sm:overflow-hidden">
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          <DialogHeader className="space-y-2 sm:space-y-4">
            ...
            <DialogTitle className="text-xl sm:text-2xl font-black text-center">Security Verification</DialogTitle>
            
            <div className="bg-secondary/50 border border-border p-3 sm:p-5 rounded-2xl text-center space-y-1 sm:space-y-2 relative overflow-hidden group">
               ...
               <p className="text-[9px] sm:text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em]">Live Demo Verification Code</p>
               <p className="text-3xl sm:text-5xl font-black text-primary tracking-[0.2em] font-mono drop-shadow-sm select-all">
                 {generatedOtp || '------'}
               </p>
          ...
          <div className="flex flex-col gap-4">
            <Input 
              className="text-center h-12 sm:h-16 text-2xl sm:text-3xl font-black tracking-[0.3em] rounded-xl border-border bg-secondary/30 text-foreground focus-visible:ring-primary focus-visible:ring-offset-2 transition-all shadow-inner"
              placeholder="000000" 
              value={otp} 
            ...
            />
            <Button 
              className="w-full h-12 sm:h-14 rounded-xl bg-primary text-primary-foreground hover:opacity-90 font-black text-lg transition-all active:scale-95 disabled:opacity-50"
